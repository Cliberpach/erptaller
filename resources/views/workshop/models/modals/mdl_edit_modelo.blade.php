    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @include('workshop.models.forms.form_edit_modelo')
            </div>
...
<script>
    ...
    async function openMdlEditMarca(modeloId) {
        parametrosMdlEditColor.id = modeloId;

        const data = await getModelo(modeloId);
        if (!data) return;
    ...
    function eventsMdlEditMarca() {
        document.querySelector('#form_edit_color').addEventListener('submit', (e) => {
            e.preventDefault();
            actualizarMarca(e.target);
        })
    }

    function pintarModeloEdit(modelo) {
        document.querySelector('#description_edit').value = modelo.description;
        window.brandEditSelect.setValue(modelo.brand_id);

    }

    async function getModelo(modeloId) {
        try {
            mostrarAnimacion1();
            toastr.clear();
            const res = await axios.get(route('tenant.taller.modelos.getModelo', modeloId));
    ...
    function actualizarMarca(formActualizarMarca) {
        ...
        `,
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    const res = await axios.post(route('tenant.taller.modelos.update', parametrosMdlEditColor
                        .id), formData);
