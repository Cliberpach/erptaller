...
namespace App\Http\Requests\Tenant\Accounts\SupplierAccount;
...
class PayStoreRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'cuenta' => array_merge(
                $this->input('metodo_pago') != 1 ? ['required'] : ['nullable'],
                $this->input('metodo_pago') != 1
                    ? [
                        Rule::exists('payment_method_accounts', 'bank_account_id')->where(function ($query) {
                            $query->where('payment_method_id', $this->input('metodo_pago'));
                        }),
                    ]
                    : []
            ),
            'nro_operacion' => [
                Rule::requiredIf(fn() => $this->input('metodo_pago') != 1),
                'nullable',
                'string',
                'max:20',
            ],
    ...
    public function messages()
    {
        return [
            ...
            'metodo_pago.required' => 'El modo de pago es obligatorio.',
            'metodo_pago.exists' => 'El modo de pago seleccionado no es válido o no está activo.',
    ...
    protected function failedValidation(Validator $validator)
    {
        throw new HttpResponseException(response()->json([
            'success' => false,
            'message' => 'Errores de validación al pagar cuenta proveedor.',
